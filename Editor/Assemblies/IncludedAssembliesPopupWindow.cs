...
namespace Editor.Assemblies
{
    ...
    /// </remarks>
    internal class IncludedAssembliesPopupWindow : PopupWindowContent
    {
        private const float kWindowHeight = 221;
        ...
        /// </remarks>
        private readonly IncludedAssembliesTreeView m_TreeView;
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            var selectLabelWidth = EditorStyles.boldLabel.CalcSize(Styles.SelectLabel).x;
            var selectAllWidth = EditorStyles.miniButton.CalcSize(Styles.SelectAllButtonLabel).x;
            var selectAssetsWidth = EditorStyles.miniButton.CalcSize(Styles.SelectAssetsButtonLabel).x;
            var selectPackagesWidth = EditorStyles.miniButton.CalcSize(Styles.SelectPackagesButtonLabel).x;
            var deselectAllWidth = EditorStyles.miniButton.CalcSize(Styles.DeselectAllButtonLabel).x;

            var searchRect = new Rect(border, topPadding, rect.width - border * 2, searchHeight);
            var selectLabelRect = new Rect(border, topPadding + searchHeight + border, selectLabelWidth, searchHeight);
            var selectAllRect = new Rect(border + selectLabelWidth + border, topPadding + searchHeight + border,
                selectAllWidth, buttonHeight);
            var selectAssetsRect = new Rect(border + selectLabelWidth + border + selectAllWidth + border,
                topPadding + searchHeight + border, selectAssetsWidth, buttonHeight);
            var selectPackagesRect =
                new Rect(border + selectLabelWidth + border + selectAllWidth + border + selectAssetsWidth + border,
                    topPadding + searchHeight + border, selectPackagesWidth, buttonHeight);
            var deselectAllRect = new Rect(rect.width - border - deselectAllWidth, topPadding + searchHeight + border,
                deselectAllWidth, buttonHeight);
            var remainingRect = new Rect(border, remainTop, rect.width - border * 2, rect.height - remainTop - border);
            ...
            if (GUI.Button(selectAllRect, Styles.SelectAllButtonLabel, EditorStyles.miniButton)) m_TreeView.SelectAll();
        ...
        private static class Styles
        {
            ...
            public static readonly GUIContent SelectAllButtonLabel = EditorGUIUtility.TrTextContent("All",
                "Click this to select and include all the assemblies in the project. This includes both the assemblies under the 'Assets' folder and packages.\n\nIf searching, it will apply only to the assemblies visible in the list.");

            public static readonly GUIContent SelectAssetsButtonLabel = EditorGUIUtility.TrTextContent("Assets",
                "Click this to select and include only the assemblies under the 'Assets' folder.\n\nIf searching, it will apply only to the assemblies visible in the list.");

            public static readonly GUIContent SelectPackagesButtonLabel = EditorGUIUtility.TrTextContent("Packages",
                "Click this to select and include only the Packages' assemblies.\n\nIf searching, it will apply only to the assemblies visible in the list.");

            public static readonly GUIContent DeselectAllButtonLabel = EditorGUIUtility.TrTextContent("Deselect All",
                "Click this to deselect and exclude all the assemblies.\n\nIf searching, it will apply only to the assemblies visible in the list.");
        }
    }
}